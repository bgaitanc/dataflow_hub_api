    ...
    : IAuthService
{
    ...
    public async Task<AuthResponseDto> LoginAsync(LoginDto loginDto)
    {
        ...
        {
            throw new DataFlowHubException(HttpStatusCode.Unauthorized, "Credenciales inválidas.");
        }
    ...
    public async Task<AuthResponseDto> RefreshTokenAsync(RefreshTokenRequestDto request)
    {
        ...
        if (principal == null)
            throw new DataFlowHubException(HttpStatusCode.Unauthorized, "Token inválido.");

        var userId = principal.FindFirst(JwtRegisteredClaimNames.Sub)?.Value
                     ?? principal.FindFirst(ClaimTypes.NameIdentifier)?.Value;

        if (userId == null) throw new DataFlowHubException(HttpStatusCode.Unauthorized, "Token inválido.");

        var user = await userManager.FindByIdAsync(userId);
        if (user == null || user.RefreshToken != request.RefreshToken || user.RefreshTokenExpiryTime <= DateTime.UtcNow)
        {
            throw new DataFlowHubException(HttpStatusCode.Unauthorized, "Refresh token inválido o expirado.");
        }
