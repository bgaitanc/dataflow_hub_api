...
/// </summary>
public class DataFlowHubControllerBase : ControllerBase
{
    /// <summary>
    ...
    /// </returns>
    protected async Task<ActionResult<DataFlowHubGenericResponse>> ExecuteServiceAsync(Func<Task> action)
    {
        try
        {
            await action.Invoke();
        ...
        catch (DataFlowHubException dfhEx)
        {
            return GenerateActionResult(new DataFlowHubGenericResponse
            {
                StatusCode = dfhEx.StatusCode,
                Success = false,
                Message = dfhEx.Message
            });
        }
        catch (Exception e)
        {
        ...
        HttpStatusCode status = HttpStatusCode.OK)
    {
        try
        {
            ...
            return GenerateActionResult(new DataFlowHubGenericResponse<T>
            {
                StatusCode = status,
                Success = true,
                Data = data!
    ...
    private ObjectResult GenerateActionResult(DataFlowHubGenericResponse response)
    {
